...
-   HTTP 프로토콜의 인프라를 그대로 사용하므로 REST AP 사용을 위한 별도의 인프라를 구출할 필요가 없다.
...
-   이러한 멀티 플랫폼에 대한 지원을 위해 서비스 자원에 대한 아키턱체를 세우고 이용하는 방법을 모색한 결과 REST에 관심을 가지게 되었다.
...
-   사내 시스템들도 REST 기반으로 시스템을 분상해 확장성과 재사용성을 높여 유지보수 및 운용을 편리하게 할 수 있다.
